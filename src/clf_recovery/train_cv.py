...
for it in range(args.maxiter):
	...
	ssim_loss = solver.net.blobs['loss_ssim'].data[0]
	fid = open(log_path+'ssim_train_log.txt','a')
	fid.write(str(ssim_loss)+'\n')
	fid.close()

	if it%100 == 0:
		print it, train_loss, ssim_loss
	...
	if (it % test_interval == 0):
		...
		#print 'batchsize', batch_size, test_iters
		for i in range(test_iters):
			solver.test_nets[0].forward()
			test_loss += solver.test_nets[0].blobs['loss'].data[0]
			ssim_loss += solver.test_nets[0].blobs['loss_ssim'].data[0]
			...
			if show_results and it%show_interval==0:
				...
				#center_views = solver.test_nets[0].blobs['crop_label'].data[...]
