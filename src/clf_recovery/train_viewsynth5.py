...
show_interval = 1000
print "training........"
for it in range(1,args.maxiter):
	#print it, 'before'
	...
	if (it % test_interval == 0):
		...
		for i in range(test_iters):
			...
			if show_results and it%show_interval==0:
				...
				pred_disp = solver.test_nets[0].blobs['warped_centerview'].data[...]
				img_save(pred_disp[idxs],it,i*1,'cwarpedview_')

				pred_disp = solver.test_nets[0].blobs['disparity'].data[...]
				img_save(pred_disp[idxs],it,i*1,'disparity_')

				reconst_views = solver.test_nets[0].blobs['iconv5'].data[...]
				img_save(reconst_views[idxs],it,i*1,'dreconst_')

				actual_views = solver.test_nets[0].blobs['cropped_label'].data[...]
