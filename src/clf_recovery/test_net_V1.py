...
path = '/home/susmitha/clf_recovery/models/view_synth2/'
...
proto = 'models/view_synth2/deploy.prototxt'
model = 'models/view_synth2/clf_iter_90000.caffemodel'
...
for img_name in img_list:
	...
	for viewLF in selLF:
		...
		for batch_id in batch_list:
			...
			warp_reconst = net.blobs['warped_img'].data.copy() # B, 3, odP, odP
		...
		imsave(path+str(count).zfill(3)+'d_centerv.png', refView)
		imsave(path+str(count).zfill(3)+'f_trv.png', trView)
		imsave(path+str(count).zfill(3)+'b_disparity.png', disp_reconst)
		imsave(path+str(count).zfill(3)+'c_warpedcv.png', warp_reconst)
		np.save(path+str(count).zfill(3)+'_disparray.np', disp_reconst)
