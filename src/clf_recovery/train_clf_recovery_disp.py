	...
	if (it % test_interval == 0):
		...
		for i in range(test_iters):
			...
			if show_results and it%show_interval==0:
				...
				center_views = solver.test_nets[0].blobs['cropped_centerview'].data[...]
				img_save(center_views[idxs],-1,i*1,'bcenterview_')

				pred_disp = solver.test_nets[0].blobs['disparity'].data[...]
				img_save(pred_disp[idxs],it,i*1,'cdisparity_')

				reconst_views = solver.test_nets[0].blobs['warped_centerview'].data[...]
