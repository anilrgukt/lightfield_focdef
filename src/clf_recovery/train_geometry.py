...
for it in range(args.maxiter):
	...
	train_loss = solver.net.blobs['warp_error'].data
	...
	if (it % test_interval == 0):
		...
		for i in range(test_iters):
			...
			test_loss += solver.test_nets[0].blobs['warp_error'].data
			...
			if show_results and it%show_interval==0:
				...
				img_save(center_views[idxs],-1,i*1,'bcenterview_')

				pred_disp = solver.test_nets[0].blobs['back_warp'].data[...]
				img_save(pred_disp[idxs],it,i*1,'cwarpedview_trview')

				pred_disp = solver.test_nets[0].blobs['h_flow'].data[...]
				img_save(pred_disp[idxs],it,i*1,'h_flow_')

				pred_disp = solver.test_nets[0].blobs['v_flow'].data[...]
				img_save(pred_disp[idxs],it,i*1,'v_flow_')

				#reconst_views = solver.test_nets[0].blobs['iconv5'].data[...]
				#img_save(reconst_views[idxs],it,i*1,'dreconst_')

				actual_views = solver.test_nets[0].blobs['label_tr'].data[...]
				img_save(actual_views[idxs],-1,i*1,'etopright_')
