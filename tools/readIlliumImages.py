...
class illiumTools():
	...
	def extract_patches_strided(self, lf, stride):
		...
		patches = []
		r_list = range(0,lf.shape[2]-P+1,S) 
		c_list = range(0,lf.shape[3]-P+1,S)
	...
	def extract_patches(self, lf):
  	...
  	def combine_patches(self, patches,rlist,clist):
		...
		count[count==0]=1
		return img/count.astype(float) # avg for overlapping patches
	...
	def getCodedImg(self, inputLF, code):
		in_shape = inputLF.shape # N,v1,v2,h,w,3
